...
Text inside colored shapes must be readable. Use `#143054` (navy) for all text inside filled shapes. Never use the zone's stroke color for text sitting on that zone's background. Reserve the zone's strokeColor for shape borders and arrows only. Default arrow strokeColor: `#5B8FB9`.
...
## Color System (aligned with 5PennyAi brand)

| Zone | Use for | strokeColor | backgroundColor |
|------|---------|-------------|-----------------|
| Blue (input/source) | Données d'entrée, sources, points de départ | `#5B8FB9` | `#E3EEF6` |
| Orange (action/process) | Actions, processus, étapes clés, CTA | `#D4713B` | `#FFF0E0` |
| Navy (concept clé) | Concepts importants, headers, conteneurs principaux | `#143054` | `#DDE4EC` |
| Coral (problème/alerte) | Problèmes, alertes, points d'attention | `#C4553A` | `#FDE8E3` |
| Lavender (infra/support) | Infrastructure, éléments de support, contexte | `#7B6B9E` | `#EEEAF5` |
| Surface (neutre) | Éléments neutres, annotations, fond | `#8C8578` | `#F7F5F2` |

For nested elements, vary the fill intensity:
- Outer: lightest (e.g., `#E3EEF6`)
- Inner: medium (e.g., `#B5D2E8`)
- Deep inner: light-medium (e.g., `#C8DFF0`)
